...
const LEGAL_TEXTS = {
  ...
  privacy: {
    ...
    html: `
      ...
      <h4>제2조 (대화 데이터 자체 서버 미저장 원칙 및 외부 AI 전송 안내)</h4>
      <ul>
        <li><strong>자체 영구 저장 일절 없음:</strong> 톡스캐너 서비스 제공자는 대화 원문이나 분석 결과를 자체 웹 서버, 파일 시스템, 데이터베이스(DB)에 일절 영구 저장하지 않으며 실시간 연산 후 즉시 소멸됩니다.</li>
        <li><strong>외부 AI 모델 API 전송 및 처리:</strong> 본 서비스는 고도화된 언어 모델 추론을 위해, 사전 가명화 및 비식별 마스킹 처리된 텍스트를 글로벌 AI 제공사(Google Gemini API, OpenAI 등)의 API 서버로 암호화(HTTPS) 전송하여 분석합니다.</li>
        <li><strong>AI 제공사의 데이터 처리 정책:</strong> 전송된 데이터는 각 AI 제공사의 정책에 따라 서비스 악용 방지·모니터링 목적으로 제공사 측에 일정 기간 임시 보관될 수 있습니다. 상용 엔터프라이즈 No-Training 규정에 따라 인공지능 모델 학습(Training)에는 사용되지 않으나, 이용자는 기밀 정보나 극히 민감한 사생활 정보의 입력을 지양해야 합니다.</li>
      </ul>
      ...
      <ul>
        <li><strong>수탁자 및 인프라 제공사:</strong> Google LLC (Gemini API), OpenAI LLC 등 글로벌 AI 클라우드 제공사</li>
        <li><strong>위탁 업무:</strong> 익명화된 대화 텍스트의 언어 패턴 분석 및 성향·호감도 통계 지표 산출</li>
        <li><strong>이전되는 항목:</strong> 실명 및 민감 개인정보가 제거·가명화된 텍스트 데이터</li>
        <li><strong>이전 국가 및 시점:</strong> 미국 등 클라우드 리전, 분석 요청 시 실시간 통신 후 자체 서버에서는 즉시 소멸</li>
      </ul>
